...
class NewResto {
    constructor(name, adress, rate,url)
    {
        this.name=name;
        this.adress=adress;
        this.rate= rate;
    ...
    div(){
        ...
        var newRestoAdress = $('<p></p>').html(this.adress+'</br>')
        var titleNewResto = $('<h4></h4>').addClass('title');
        $('<a href="#restoDescription"></a>').text(this.name).appendTo(titleNewResto)
        ...
        var imgNewResto = $('<img/>').attr('src', this.url).css('margin-right', '5px');
        ...
        for (let i = 0; i < 5; i++) {
          ...
          if (Math.floor(parseInt(this.rate))> i) {
    ...
    card(){
        ...
        var titleCard = $('<h4></h4>').text(this.name).addClass('title');
        titleCard.appendTo('#restoDescription');
        var paraCard= $('<p></p>').html(this.adress +'</br>');
        paraCard.appendTo('#restoDescription');
        for (let i = 0; i < 5; i++) {
          var starCard = $('<i class="fas fa-star"></i>');
          starCard.appendTo('#restoDescription p');
          if (Math.floor(this.rate)>i) {
            starCard.css('color', 'yellow');
          }
        }
        $('<span></span>').text('note : '+this.rate).insertAfter($('#restoDescription i:last'));
        var imgResto = $('<img/>').attr('src', this.url).css('width', '300px').css('height','200px');
...
function geocodeLatLng(geocoder, map, infoWindow) {
  ...
  geocoder.geocode({'location': input}, function(results, status) {
    ...
    var newRestoInst= new NewResto(nameNewResto,geoCodeAdress,rateNewResto,newRestoUrl)
    newRestoInst.div();
    var divList = $('#restList a');
    divList.click(function() {
     newRestoInst.card();
        console.log(NewResto.name);
    });
  });
}
