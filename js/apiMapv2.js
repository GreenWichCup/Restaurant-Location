...
function geocodeLatLng(geocoder, map, infoWindow) {
  ...
  geocoder.geocode({'location': input}, function(results, status) {
    ...
    nameNewResto= prompt("Entrez le nom du restaurant:");
    rateNewResto= prompt("Entrez une note:");
...
function restoDes(e){
    ...
    var titleCard = $('<h4></h4>').text(clickValue).addClass('title');
    titleCard.appendTo('#restoDescription');
    var paraCard= $('<p></p>').html(geoCodeAdress+'</br>');
...
class NewResto {
    constructor(name, adress, rate,img)
    {
        this.name=name;
        this.adress=adress;
        this.rate= rate;
